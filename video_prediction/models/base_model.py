...
class BaseVideoPredictionModel(object):
    ...
    def restore(self, sess, checkpoints):
        ...
        if checkpoints:
            # possibly restore from multiple checkpoints.b useful if subset of weights
...
class VideoPredictionModel(BaseVideoPredictionModel):
    ...
    def generator_loss_fn(self, inputs, outputs, targets):
        ...
        if hparams.l1_weight or hparams.l2_weight or hparams.vgg_cdist_weight or hparams.l2_NDVI_weight:
        ...
        if hparams.variability_weight:
            gen_variability_loss = vp.losses.variability_loss(gen_images, target_images)
            gen_losses["gen_variability_loss"] = (gen_variability_loss, hparams.variability_weight)
        if hparams.l2_NDVI_weight:
            gen_l2_NDVI_loss = vp.losses.l2_NDVI_loss(gen_images, target_images)
            gen_losses["gen_l2_NDVI_loss"] = (gen_l2_NDVI_loss, hparams.l2_NDVI_weight)
...
#             if self.hparams.kl_goal is not None:
#                 if self.g_losses['kl_loss'] > self.hparams.kl_goal:
#                     self.kl_weight =- self.hparams.kl_goal_step
#                 if self.g_losses['kl_loss'] < self.hparams.kl_goal:
#                     self.kl_weight =+ self.hparams.kl_goal_step
