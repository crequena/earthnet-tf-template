...
def structural_similarity_np(true, pred, K1=0.01, K2=0.03, sigma=1.5, win_size=None,
    ...
    true = np.expand_dims(true[:,:,:,:,2], axis=-1)
    pred = np.expand_dims(pred[:,:,:,:,2], axis=-1)
...
def binary_accuracy_tensor(true, pred, keep_axis=None):
    """
    Converts input into binary (0 if minor than 1 and 1 if higher). 
    And checks whether the prediction and the target match.
    Typically used for precipitation occurrance accuracy.
    Computes it over central channel (green=rain)
    ...
    """
    _, true, _ = true
    true = tf.where(true < 1, tf.zeros_like(true), tf.ones_like(true))
    
    _ , pred, _ = tf.unstack(tf.convert_to_tensor(pred), axis=4)
    pred = tf.where(pred < 1, tf.zeros_like(pred), tf.ones_like(pred))
    
    error = tf.where(true == pred, tf.ones_like(true), tf.zeros_like(true))
    return tf.reduce_mean(error, axis=_axis(keep_axis, error.shape.ndims))

def binary_accuracy(true, pred, keep_axis=None):
    """
    Converts input into binary (0 if minor than 1 and 1 if higher). 
    And checks whether the prediction and the target match.
    Typically used for precipitation occurrance accuracy.
    ...
    Returns:
        mean squared error between ground truth and predicted image.
    """
    #true = true[:,:,:,:,1] #Re convert 0-1 to 0-6.34. The real range of cubic rooted precipitation
    ...
    error = np.where(true == pred, 1, 0)
    return np.mean(error, axis=_axis(keep_axis, error.ndim))

'''
def rain_mae(true, pred, keep_axis=None):
    
    trueocc = true
    predocc = pred
    
    trueocc[trueocc<0.1575] = 0
    predocc[predocc<0.1575] = 0
    trueocc[trueocc>0.1575] = 1
    predocc[predocc>0.1575] = 1
    
    #true =  (true[:,:,:,:,1]*6.3487773)
    #true = np.expand_dims(true, axis=-1)
    
    #pred =  (pred[:,:,:,:,1]*6.3487773)
    #pred = np.expand_dims(pred, axis=-1)
    
    trueocc
    predocc
    
    pred = np.where(trueocc == 1, pred, 0)
    
    error = np.abs(np.power(true,3) - np.power(pred,3))
...
def vgg_cosine_distance_np(image0, image1, keep_axis=None):
    return 1.0 - vgg_cosine_similarity_np(np.expand_dims(image0[:,:,:,:,2], axis=-1), np.expand_dims(image1[:,:,:,:,2], axis=-1), keep_axis=keep_axis)

'''
ACTUAL VGG DISTANCE SIM
def vgg_cosine_distance_np(image0, image1, keep_axis=None):
