...
# #         forgoten = self._conv2d(array_ops.concat([c, math_ops.sigmoid(f)], -1))
# #         inputted = self._conv2d(array_ops.concat([math_ops.sigmoid(i), g], -1))
# #         new_c = forgoten + inputted
        
#         new_c = (c * math_ops.sigmoid(f + self._forget_bias)
#                  + math_ops.sigmoid(i) * g) #HERE CONV!
#         if self._normalizer_fn:
#             new_c = self._norm(new_c, "state")
        
# #         new_h = forgoten = self._conv2d(array_ops.concat([self._activation_fn(new_c), math_ops.sigmoid(o)], -1))

            
#         new_h = self._activation_fn(new_c) * math_ops.sigmoid(o)  #HERE CONV!
...
class FullyConv2DLSTMCell(rnn_cell_impl.RNNCell):
    """2D Convolutional LSTM cell with convolutional gate masking instead of hadamard product.
    ...
    def __init__(self, input_shape, filters, kernel_size,
        ...
        """
        super(FullyConv2DLSTMCell, self).__init__(_reuse=reuse)
...
class FullyConv2DGRUCell(tf.nn.rnn_cell.RNNCell):
    """2D Convolutional GRU cell with (optional) normalization.
    ...
    def __init__(self, input_shape, filters, kernel_size,
                 ...
                 normalizer_fn=None, separate_norms=True,
                 bias_initializer=None, reuse=None):
        super(FullyConv2DGRUCell, self).__init__(_reuse=reuse)
        self._input_shape = input_shape
        self._filters = filters
        #self._kernel_size = list(kernel_size) if isinstance(kernel_size, (tuple, list)) else [kernel_size] * 2
        kernel_size = (3,3)
        self._kernel_size = list(kernel_size) if isinstance(kernel_size, (tuple, list)) else [kernel_size] * 2
        kernel_size2 = (1,1)
        self._kernel_size2 = list(kernel_size2) if isinstance(kernel_size2, (tuple, list)) else [kernel_size2] * 2
    ...
    def __conv2d(self, inputs, output_filters, bias_initializer):
        input_shape = inputs.get_shape().as_list()
        kernel_shape = list(self._kernel_size2) + [input_shape[-1], output_filters]
        kernel = vs.get_variable("kernel1", kernel_shape, dtype=dtypes.float32,
                                 initializer=init_ops.truncated_normal_initializer(stddev=0.02))
        outputs = nn_ops.conv2d(inputs, kernel, [1] * 4, padding='SAME')
        if not self._normalizer_fn:
            bias = vs.get_variable('bias', [output_filters], dtype=dtypes.float32,
    ...
    def ___conv2d(self, inputs, output_filters, bias_initializer):
        input_shape = inputs.get_shape().as_list()
        kernel_shape = list(self._kernel_size2) + [input_shape[-1], output_filters]
        kernel = vs.get_variable("kernel2", kernel_shape, dtype=dtypes.float32,
                                 initializer=init_ops.truncated_normal_initializer(stddev=0.02))
        outputs = nn_ops.conv2d(inputs, kernel, [1] * 4, padding='SAME')
        if not self._normalizer_fn:
            bias = vs.get_variable('bias', [output_filters], dtype=dtypes.float32,
    ...
    def ____conv2d(self, inputs, output_filters, bias_initializer):
        input_shape = inputs.get_shape().as_list()
        kernel_shape = list(self._kernel_size2) + [input_shape[-1], output_filters]
        kernel = vs.get_variable("kernel3", kernel_shape, dtype=dtypes.float32,
                                 initializer=init_ops.truncated_normal_initializer(stddev=0.02))
        outputs = nn_ops.conv2d(inputs, kernel, [1] * 4, padding='SAME')
        if not self._normalizer_fn:
            bias = vs.get_variable('bias', [output_filters], dtype=dtypes.float32,
    ...
    def call(self, inputs, state):
        ...
        with vs.variable_scope('candidate'):
            #inputs = array_ops.concat([inputs, r * state], axis=-1)
            r_times_state = self.____conv2d(array_ops.concat([r, state], axis=-1), self._filters, bias_ones)
            inputs = array_ops.concat([inputs, r_times_state], axis=-1)
            candidate = self._conv2d(inputs, self._filters, bias_zeros)
            if self._normalizer_fn:
                candidate = self._norm(candidate, "state", bias_zeros)
        ...
        #new_h = u * state + (1 - u) * c #Make conv
        
        u_times_state = self.__conv2d(array_ops.concat([u, state], axis=-1), self._filters, bias_ones)
        u_times_c = self.___conv2d(array_ops.concat([1 - u, c], axis=-1), self._filters, bias_ones)
        new_h = u_times_state + u_times_c
