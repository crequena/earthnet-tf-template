...
def main():
    ...
    parser.add_argument("--logs_dir", default='logs', help="ignored if checkpoint is specified")
    parser.add_argument("--checkpoint", help="output directory where json files, summary, model, gifs, etc are saved. "
    ...
    parser.add_argument("--resume", action='store_true', help='resume from lastest checkpoint in checkpoint.')
    ...
    parser.add_argument("--dataset_hparams_dict", type=str, help="directory to dataset_hparams.json file of dataset hyperparameters")
    parser.add_argument("--model", type=str, help="model class name")
    parser.add_argument("--model_hparams", type=str, help="a string of comma separated list of model hyperparameters")
    parser.add_argument("--model_hparams_dict", type=str, help="directory to model_hparams.json file of model hyperparameters")
    ...
    if args.checkpoint is None:
        ...
        for t in ('model=%s,%s' % (args.model, args.model_hparams)):
            ...
            if t in '[]':
                t = ''
            model_fname += t
        args.checkpoint = os.path.join(args.logs_dir, model_fname)

    dataset_hparams_dict = {}
    model_hparams_dict = {}
    if args.dataset_hparams_dict:
        with open(os.path.join(args.dataset_hparams_dict,'dataset_hparams.json')) as f:
            dataset_hparams_dict.update(json.loads(f.read()))
    if args.model_hparams_dict:
        with open(os.path.join(args.model_hparams_dict,'model_hparams.json')) as f:
    ...
    with open(os.path.join(args.checkpoint, "dataset_hparams.json"), "w") as f:
        f.write(json.dumps(train_dataset.hparams.values(), sort_keys=True, indent=4))
    with open(os.path.join(args.checkpoint, "model_hparams.json"), "w") as f:
        f.write(json.dumps(train_model.hparams.values(), sort_keys=True, indent=4))

    if args.gif_freq:
        epsilon = tf.constant(0.000000001, dtype=tf.float32)
        ...
        if args.dataset == 'earthnet':
            ##Generated NDVI
            r, b, g, nir, _, _, _, _, _, _, _, _ =tf.unstack(val_tensors['gen_images_vis'], axis=4, name='unstack_gen')
            ndvi = (nir-r)/(nir+r+epsilon)
            val_tensors['gen_ndvi'] = tf.stack([ndvi, ndvi, ndvi], axis=4, name='stack_gndvi')

            ##Targets predictors and cloud mask (with context)
            context_targets = val_model.inputs['images'][:, :val_model.hparams.context_frames]
            val_tensors['targets']  = tf.concat([context_targets, val_model.targets], axis=1)

            r, g, b, nir, hr_dem, rr, pp, tg, tn, tx, meso_dem, mask = tf.unstack(val_tensors['targets'] , axis=4, name='unstack_predictors')
            t_ndvi = (nir-r)/(nir+r+epsilon)
            val_tensors['target_ndvi'] = tf.stack([t_ndvi, t_ndvi, t_ndvi], axis=4, name='stack_tndvi')
            val_tensors['target_weather'] = tf.stack([pp, tg, rr], axis=4, name='stack_weather')
            val_tensors['target_hr_elevation'] = tf.stack([hr_dem, hr_dem, hr_dem], axis=4, name='stack_hr_elevation')
            val_tensors['target_meso_elevation'] = tf.stack([meso_dem, meso_dem, meso_dem], axis=4, name='stack_meso_elevation')
            val_tensors['target_mask'] = tf.stack([mask, mask, mask], axis=4, name='stack_mask')
    ...
    if args.summary_freq or args.image_summary_freq or args.eval_summary_freq:
        summary_writer = tf.summary.FileWriter(args.checkpoint)
    ...
    with tf.Session(config=config) as sess:
        ...
        for step in range(-1, max_steps - start_step):
            ...
            if should(args.progress_freq):
                ...
                for name, loss in itertools.chain(results['d_losses'].items(), results['g_losses'].items()):
                    ...
                    if math.isnan(loss[0]) or math.isnan(loss[1]):
                        print("NaN loss found. Exiting training")
                        exit()
                if isinstance(train_model.learning_rate, tf.Tensor):
            ...
            if should(args.save_freq):
                ...
                saver.save(sess, os.path.join(args.checkpoint, "model"), global_step=global_step)
                print("done")

            if should(args.gif_freq):
                image_dir = os.path.join(args.checkpoint, 'images')
